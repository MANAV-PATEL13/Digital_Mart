...
public:
    ...
    void viewproduct() {
        ...
        for (size_t i = 0; i < cart.size(); i++) {
            cout << (i + 1) << "\t" << cart[i].first << "\t" << cart[i].second << endl;
        }
        cout << "-----------------------------------\n";
    }
};
...
public:
    ...
    void Order() {
        ...
        if (number < 1 || number > cart.size()) {  // Validate user input
    ...
    void Vieworder() {
        ...
        cout << "Total Amount to Pay: " << grandTotal << endl;
        double grandTotal = 0;
    ...
    void generatebill(string username) {
          ...
          for (char& c : billId) {
              if (c == ' ') c = '_';   // Replace space with underscore
              if (c == ':') c = '-';   // Replace colon with hyphen
          }
...
void addEmployee(const string& username, const string& password) {
          ...
          if (employeeFile) {
              ...
              employeeFile.close();
              cout << "Employee '" << username << "' added successfully." << endl;
          } else {
